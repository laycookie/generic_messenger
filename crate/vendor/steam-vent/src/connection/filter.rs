                            ...
                            filter_send.oneshot_kind_filters.remove(&message.kind)
                        {
                                ...
                                message.into_message::<ServiceMethodNotification>()
                            {
                                debug!(
                                    ...
                                    // Local fork: dump the raw body so unpublished
                                    // service-method payloads (VoiceChatClient.*)
                                    // can be recovered. Lazily evaluated, so it's
                                    // free unless this debug span is enabled.
                                    body = %notification.body_hex(),
