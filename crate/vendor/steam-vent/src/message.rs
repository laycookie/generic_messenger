...
impl ServiceMethodNotification {
    ...
    /// Capture helper (local fork): the raw protobuf body as a hex string, for
    /// recovering unpublished service-method payloads such as `VoiceChatClient.*`.
    /// See `interface/steam/VOICE_PROTOCOL.md`.
    pub(crate) fn body_hex(&self) -> String {
